        ...
        if serializer.is_valid():
            ...
            for animal in animals:
                ...
                if animal.shelter:
                    action.send(
                        self.request.user,
                        verb="sheltered animal in",
                        target=animal,
                        action_object=animal.shelter,
                    )
                    action.send(
                        self.request.user,
                        verb="sheltered animal",
                        target=animal.shelter,
                        action_object=animal,
                    )

                if animal.room:
                    action.send(
                        self.request.user,
                        verb="roomed animal in",
                        target=animal,
                        action_object=animal.room,
                    )
                    action.send(
                        self.request.user,
                        verb="roomed animal",
                ...
                for key, image_data in images_data.items():
                    ...
                    AnimalImage.objects.create(
                        image=image_data, animal=animal, category=category
                    )
    ...
    def perform_update(self, serializer):
        if serializer.is_valid():
            ...
            serializer.validated_data[
                "owners"
            ] = serializer.instance.owners.values_list("id", flat=True)

            # Mark as SHELTERED if we receive shelter field and it's not already in a shelter.
            if (
                serializer.validated_data.get("shelter")
                and not serializer.instance.shelter
            ):
                serializer.validated_data["status"] = "SHELTERED"
                serializer.validated_data["intake_date"] = datetime.now()
                action.send(
                    ...
                    action_object=serializer.validated_data.get("shelter"),
                )
                action.send(
                    self.request.user,
                    verb="sheltered animal",
                    target=serializer.validated_data.get("shelter"),
            ...
            if serializer.validated_data.get(
                "shelter"
            ) and serializer.instance.shelter != serializer.validated_data.get(
                "shelter"
            ):
                action.send(
                    self.request.user,
            ...
            if serializer.instance.shelter and (
                serializer.instance.shelter
                != serializer.validated_data.get("shelter", serializer.instance.shelter)
            ):
                action.send(
                    self.request.user,
                    verb="removed animal",
                    target=serializer.instance.shelter,
                    action_object=serializer.instance,
                )

            # If animal had a room and now doesn't or has a different room.
            if serializer.instance.room and (
                not serializer.validated_data.get("room")
                or serializer.instance.room != serializer.validated_data.get("room")
            ):
                action.send(
                    self.request.user,
                    verb="removed animal",
                    target=serializer.instance.room,
                    action_object=serializer.instance,
                )
                action.send(
                    self.request.user,
                    verb="removed animal",
                    target=serializer.instance.room.building,
                    action_object=serializer.instance,
                )

            # If animal is added to a new room from no room or a different room.
            if serializer.validated_data.get("room") and (
                serializer.instance.room != serializer.validated_data.get("room")
            ):
                action.send(
                    self.request.user,
                    verb="roomed animal in",
                    target=serializer.instance,
                    action_object=serializer.validated_data.get("room"),
                )
                action.send(
                    self.request.user,
                    verb="roomed animal",
                    target=serializer.validated_data.get("room"),
                    action_object=serializer.instance,
                )
                action.send(
                    self.request.user,
                    verb="roomed animal",
                    target=serializer.validated_data.get("room").building,
                    action_object=serializer.instance,
                )

            # Record status change if appplicable.
            if serializer.instance.status != serializer.validated_data.get(
                "status", serializer.instance.status
            ):
                new_status = serializer.validated_data.get("status")
                action.send(
                    self.request.user,
                    verb=f"changed animal status to {new_status}",
                    target=serializer.instance,
                )
            ...
            for field, value in serializer.validated_data.items():
                ...
                if (
                    field not in ["status", "room", "shelter", "order", "owners"]
            ...
            if self.request.data.get("remove_animal"):
                Animal.objects.filter(id=self.request.data.get("remove_animal")).update(
                    status="CANCELED", shelter=None, room=None
                )
            ...
            updated_images = (
                self.request.data["extra_images"].split(",")
                if self.request.data.get("extra_images", None)
                else []
            )
            ...
            images_to_delete = [
                image_to_delete.replace(settings.MEDIA_URL, "")
                for image_to_delete in set(old_images) - set(updated_images)
            ]
            AnimalImage.objects.filter(
                animal=animal, category="extra", image__in=images_to_delete
            ).delete()
            ...
            for key, image_data in images_data.items():
                ...
                if key in ("front_image", "side_image"):
                    AnimalImage.objects.create(
                        image=image_data, animal=animal, category=key
                    )
                # Otherwise create a new extra image.
                else:
                    AnimalImage.objects.create(
                        image=image_data, animal=animal, category="extra"
                    )
