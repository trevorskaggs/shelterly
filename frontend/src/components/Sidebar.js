...
const Menu = ({ state, dispatch, removeCookie, ...props }) => {
    ...
    return (
    <StyledMenu  {...props} className="flex-column" style={{ height: viewHeight, minHeight:"975px" }}>
      ...
      <Collapse in={showSearch}>
        <ListGroup variant="flush" style={{marginTop:"-5px", marginBottom:"-10px"}}>
          ...
          <ListGroup.Item action className="rounded sidebar" onClick={() => navigate(incident + '/vet/vetrequest/search')} style={{backgroundColor:path.includes("vet/vetrequest/search") ? "#444444" : "#292b2c"}}><FontAwesomeIcon className="mr-1 sidebar-icon" icon={faSearch} fixedWidth inverse/><span className="sidebar-icon">VETERINARY REQUESTS</span></ListGroup.Item>
