...
export const PersonForm = ({ id }) => {
  ...
  const is_first_responder = window.location.pathname.includes('first_responder');
        ...
        {(props) => (
          <Card border="secondary" className="mt-5">
            <Card.Header as="h5" className="pl-3">
              {' '}
              <Link href="/hotline" className="mr-3"><FontAwesomeIcon icon={faArrowAltCircleLeft} size="lg" inverse /></Link>
              {is_owner ? 'Owner' : 'Reporter'}
              {' '}
              Information
            </Card.Header>
            <Card.Body>
