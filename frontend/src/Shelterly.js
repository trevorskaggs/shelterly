...
function Shelterly() {
  ...
  const style = state.user && path !== '/' && path !== '/' + org && !path.includes('/incident/') && !path.includes('/reset_password') ? {position:"absolute", marginLeft:"335px"} : {position:"absolute", maxWidth:"100%"};

  return (
    <ThemeProvider theme={theme}>
      <Container fluid>
        <Row>
          {state.user && path !== '/' && path !== '/' + org && !path.includes('/incident/') && !path.includes('/reset_password') ?
          <span>
