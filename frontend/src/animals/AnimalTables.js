          ...
          <div className="row">
            ...
            <div className="col-sm-3">
              ...
              <Fab
                ...
                className="mb-1"
                style={{
                  width: 23, height: 23, minHeight: 23, color: '#fff', backgroundColor: '#c82333',
                }}
                title="Animal details"
                aria-label="details"
              >
                <AssignmentIcon style={{ fontSize: 10 }} />
              </Fab>
            ...
            <div className="col-sm">
              {animal.request ? (
                <div>
                  ...
                  {' '}
                  <Fab color="primary" href={`/hotline/servicerequest/${animal.request}`} className="mb-1" style={{ width: 23, height: 23, minHeight: 23 }} title="request details" aria-label="request_details"><AssignmentIcon style={{ fontSize: 10 }} /></Fab>
                </div>
              ) : ''}
              {animal.owner ? (
                <div>
                  <b>Owner:</b>
                  {' '}
                  {animal.owner_object.first_name}
                  {' '}
                  {animal.owner_object.last_name}
                  {' '}
                  {animal.owner_object.phone}
                  {' '}
                  {animal.owner_object.first_name !== 'Unknown' ? (
                    <Fab
                      href={`/hotline/owner/${animal.owner}`}
                      className="mb-1"
                      style={{
                        width: 23, height: 23, minHeight: 23, color: '#fff', backgroundColor: '#28a745',
                      }}
                      title="Owner details"
                      aria-label="owner_details"
                    >
                      <AssignmentIcon style={{ fontSize: 10 }} />
                    </Fab>
                  ) : (
                    <Fab
                      href={`/hotline/owner/edit/${animal.owner}`}
                      className="mb-1"
                      style={{
  width: 23, height: 23, minHeight: 23, color: '#fff', backgroundColor: '#28a745',
}}
                      title="Edit Owner"
                      aria-label="edit_owner"
                    >
              ...
              <div>
                <b>Location:</b>
                {' '}
                {animal.full_address ? animal.full_address : 'Unknown'}
              </div>
            </div>
          </div>
        </div>
