...
function VetRequestSearch({ incident }) {
  ...
  useEffect(() => {
    ...
    const fetchServiceRequests = async () => {
      ...
      await axios.get('/vet/api/vetrequests/?search=' + searchTerm, {
  ...
  return (
    <div className="ml-2 mr-2">
      ...
      {data.vet_requests.map((service_request, index) => (
      ...
      <p>{data.isFetching ? 'Fetching service requests...' : <span>{data.vet_requests && data.vet_requests.length ? '' : 'No Service Requests found.'}</span>}</p>
