  ...
  return (
    <>
      <h1 style={header_style}>
        Service Request #
        {data.id}
        <Link href={`/hotline/servicerequest/edit/${id}`}>
          {' '}
        ...
        <div className="row">
          <div className="col-8">
            ...
            <p className="mb-3">
              ...
              {data.reporter ? (
                <span>
                  {data.reporter_object.first_name}
                  {' '}
                  {data.reporter_object.last_name}
                  {' '}
                  <Link href={`/hotline/reporter/${data.reporter}`}>
                    {' '}
                    <FontAwesomeIcon icon={faClipboardList} inverse />
                  </Link>
                  <Link href={`/hotline/reporter/edit/${data.reporter}`}>
                    {' '}
                    <FontAwesomeIcon icon={faEdit} inverse />
                  </Link>
                </span>
            ...
            <p>
              <b>Address:</b>
              {' '}
              {data.address ? <span>{data.full_address}</span> : 'N/A'}
            </p>
            <p>
              <b>Directions:</b>
              {' '}
          ...
          <div className="col-4">
            <p>
              <b>Verbal Permission:</b>
              {' '}
              {String(data.verbal_permission)}
            </p>
            <p>
              <b>Key Provided:</b>
              {' '}
              {String(data.key_provided)}
            </p>
            <p>
              <b>Accessible:</b>
              {' '}
              {String(data.accessible)}
            </p>
            <p>
              <b>Turn Around:</b>
              {' '}
              {String(data.turn_around)}
            </p>
          </div>
        </div>
      </div>
              ...
              <div className="col-8">
                ...
                {data.recovery_time ? (
                  <p>
                    <b>Recovery Time:</b>
                    {' '}
                    <Moment format="LLL">{data.recovery_time}</Moment>
                  </p>
                ) : ''}
                {data.owner_notification_tstamp ? (
                  <p>
                    <b>Owner Notified:</b>
                    {' '}
                    <Moment format="LLL">{data.owner_notification_tstamp}</Moment>
                  </p>
                ) : ''}
              </div>
              <div className="col-4">
        ...
        ? (
          <div style={card_style} className="card card-body bg-light mx-auto">
            <p>
              <b>Animals:</b>
              {' '}
              <Link href={`/hotline/animal/new?servicerequest_id=${id}`}>
                {' '}
                <FontAwesomeIcon icon={faPlusSquare} inverse />
              </Link>
            </p>
            <span>
              {data.animals.map((animal) => (
                <li key={animal.id}>
                  ...
                  <Link href={`/animals/animal/edit/${animal.id}`}>
                    {' '}
                    <FontAwesomeIcon icon={faEdit} inverse />
                  </Link>
                </li>
