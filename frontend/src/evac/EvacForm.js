...
export const EvacTeamForm = () => {
  ...
  const teammembers = [];
  axios.get('http://localhost:8000/people/api/teammember/').then(function (response) {
    response.data.map(function(teammember){
      const obj = {value: teammember.id, label: teammember.first_name};
      teammembers.push(obj)
    });
    setData({teammembers: teammembers, isFetching: false});
  });
  const options = data.teammembers;
  return (
    <>
