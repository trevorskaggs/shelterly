...
class ServiceRequestViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        if serializer.is_valid():
            ...
            for service_request in ServiceRequest.objects.filter(incident__slug=self.request.GET.get('incident'), address=serializer.validated_data['address'], city=serializer.validated_data['city'], state=serializer.validated_data['state']):
    ...
    def perform_update(self, serializer):
        if serializer.is_valid():
            ...
            if 'address' in serializer.validated_data and 'city' in serializer.validated_data and 'state' in serializer.validated_data:
                for service_request in ServiceRequest.objects.filter(incident__slug=self.request.GET.get('incident'), address=serializer.validated_data['address'], city=serializer.validated_data['city'], state=serializer.validated_data['state']).exclude(id=self.kwargs['pk']).exclude(status='CANCELED'):
                    raise serializers.ValidationError(['Multiple service requests may not exist with the same address.', service_request.id])
            service_request = serializer.save()
